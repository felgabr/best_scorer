...
# Output
featsel_path = __CONFIG__['outputs']['feat_sel']['cramer']
# ML config
...
try:
    ...
    logger.info('Start Cramer\'s V')
    selector.split_data()
    pd_results = selector.cramers_v(excpt_list=excpt_list)
...
# ==================================================================================   
    ...
    message = f'''Dear collesgues,<br> Time elapsed doing feature selecting using Cramer\'s V
    ...
    message = f'''Dear colleagues,<br> Time elapsed in this attempt to do feature selection using Cramer\'s V
                  was {DateAux.time_elapsed(end_time, start_time)} hours.
                  <br> <br> Best Regards.''' 
    send_email(sender=dest, to=recp, cc=recp_cp, message=message, subject=subject,
                 attch=[log_file, stdlog_file])