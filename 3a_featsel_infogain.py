...
# Output
featsel_path = __CONFIG__['outputs']['feat_sel']['infogain']
# ML config
...
try:
    ...
    selector = FeatureSelector(data=df, config=config, model=rfc)
    selector.split_data()
    pd_results = selector.info_gain()
    logger.debug(pd_results)
...
# ==================================================================================
    ...
    message = f'''Dear colleagues,<br> Time elapsed doing feature selecting using info gain
    ...
    message = f'''Dear colleagues,<br> Time elapsed in this attempt to do feature selection using info gain
                  was {DateAux.time_elapsed(end_time, start_time)} hours.
                  <br> <br> Best Regards.''' 
    send_email(sender=dest, to=recp, cc=recp_cp, message=message, subject=subject,
                 attch=[log_file, stdlog_file])